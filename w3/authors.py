...
source = dict((filename, file_contents(filename)) for filename in text_files)
...
def mapfn(key, value):
    ...
    for line in value.splitlines():
        parts = line.split(":::")
        title = parts.pop().lower()
        paper = parts.pop(0) # not used?
        authors = parts.pop().split("::")
        for term in title.split():
            term = ''.join(i for i in term if i.isalnum())
            if term not in allStopWords:
                for author in authors:
                    yield author, term


def reducefn(author, wordbag):
    words = dict()
    for w in wordbag:
        if w in words:
            words[w] += 1
    ...
    wordlist = sorted(words, key = lambda key: words[key], reverse=True)
    countlist = list()
    for word in wordlist:
        countlist.append(word+","+str(words[word]))
    del wordlist[10:]
    del countlist[10:]
    return countlist
...
#yield ''.join(i for i in title if (i.isalnum()), 1
results = s.run_server(password="password")
#print words
for author in results:
    countlist = results[author]
    print author + " || " + str(results[author])
